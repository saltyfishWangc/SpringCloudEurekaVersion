#!/bin/bash
# jump to de parent diretory
cd `dirname $0`
BIN_DIR=`pwd`
cd ..
DEPLOY_DIR=`pwd`
CONF_DIR=$DEPLOY_DIR/conf
SERVER_NAME=springcloud-eureka-server
LOGS_FILE=$DEPLOY_DIR/logs
MAIN_CLASS="com.wangc.springcloud.EurekaServer8300"
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMS_OPTS -classpath $CONF_DIR:$LIB_JARS ${MAIN_CLASS} 2>&1 >> $STDOUT_FILE &
